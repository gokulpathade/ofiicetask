...
getrepodetails(USER_ID);
  ...
  const details = (USER_ID) => {
    axios
      .get(config.serverURL + "/reporting/reportingdetail/managerid"+ USER_ID, {
        headers: { token: sessionStorage["token"] },
      })
      .then((response) => {
        const result = response.data;
     ...
     <div style={{marginLeft:0,marginTop:5}}><FaIcons.FaUserFriends size={'100px'}/></div> */}
        ...
        <div style={{ marginTop: 20 }}>
          <h3 style={{ fontSize: 25, fontFamily: "cursive" }}>
           Team Application Leave Request Status
          </h3>
        </div>
        ...
        <div style={{ marginTop: 20 }}>
          <h3 style={{ fontSize: 25, fontFamily: "cursive" }}>Team Reporting Status</h3>
        </div>
        <div
          className="cantainer"
          style={{
        ...
        >
          <table className="table table-sm">
            <thead>
              <tr style={{ backgroundColor: "#8275a5", color: "white" }}>
                <th scope="col">Emp Registere Id</th>
                <th scope="col">Emp No</th>
                <th scope="col">Name</th>
            ...
            <tbody>
              {getuser.map((r) => (
                <tr key={r.Reporting_Id}>
                  {" "}
                  {/* Adding a unique key for each iteration */}
                  <td>{r.Reporting_Id}</td>
                  <td>{r.empno}</td>
                  <td>{r.firstName}</td>
                  <td>{r.Time_In}</td>
                  <td>{r.Time_Out}</td>
                  ...
                  >
                    {/* <select
                      style={{
                        Reporting_Id, Time_In, Time_Out, Date, Reason, Apply_id, Department, Creation_Date, Status, Manager_Id
                        border: "none",
                        backgroundColor: "#ff9999",
                        fontSize: "18",
                      }}
                    ...
                    > 
                      <option style={{ color: "red" }}>Pending
                        {emps.applyStatus}
                      </option>
                      {appoptions.map((item) => (
