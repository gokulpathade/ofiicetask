...
// edit method for applyleave table status updated
  ...
  const edituser = (appid) => {
   // console.log("appid : ",appid)
       ...
       axios.put(config.serverURL + '/applyLeave/edit/'+appid,body ,{
          headers: { token: sessionStorage['token'] },
          }).then((response) => {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (result['status'] === 'success') {
              ...
              console.log("specific reporting details showing :",response.data.data)
...
//Rejected data get by id 
  ...
  const deleterejecteddata = (rejectedData) => {
    for (const data of rejectedData) {
      ...
      axios
        .delete(config.serverURL + '/registration1/rejected/' + TodayDate + '/' + regid, {
          headers: { token: sessionStorage['token'] },
        })
        .then((response) => {
          const result = response.data;
 ...
 // getdetailbyidstatus(r.reportid);
      ...
      const handleInsertion = (reportingData) => {
         ...
         console.log("inside handleInsertion reporting",reportingData)
          ...
          console.log("obj",obj)
          axios.post(config.serverURL + '/timereg/timeee', obj, {
            headers: {
      ...
      // this method use for insert the record from reporting table to registration1 table
        ...
        const body = { status };
      //  console.log("bodyyyy : ", body)
        axios.put(config.serverURL + '/reporting/reportedit/' + id, body, {
          headers: { token: sessionStorage['token'] },
        }).then((response) => {
          const result = response.data
...
//registrations count details
    ...
    const getuserdetailss = () => {
      ...
      const updateNotification =()=>{   
        const body={ stauss };
       // setOpen(false);
     axios.put(config.serverURL + '/applyLeave/applyleavestatus/',body ,{
        headers: { token: sessionStorage['token'] },
        }).then((response) => {
...
<Grid item xs={2}>
<MDBCard style={{ maxWidth: '400px', backgroundColor:' #f2f2f2' ,boxShadow:2}}   >
      <MDBRow className='g-0'>
        ...
        <MDBCol md='8'>
          <MDBCardBody>
            <MDBCardTitle className='text-black' style={{fontSize:22}}>
              Total Employees</MDBCardTitle>
            <MDBCardText className='text-black' style={{fontSize:35}}>
              {registrationss}
            </MDBCardText>
            {/* <MDBCardText>
...
<MDBCard style={{ maxWidth: '400px', backgroundColor:' #f2f2f2'}}>
      <MDBRow className='g-0'>
        ...
        <MDBCol md='8'>
          <MDBCardBody>
            <MDBCardTitle className='text-black' style={{fontSize:22}}>
              Total Departments</MDBCardTitle>
            <MDBCardText className='text-black' style={{fontSize:35}}>
              {dept}
            </MDBCardText>
            {/* <MDBCardText>
...
<MDBCard style={{ maxWidth: '400px', backgroundColor:' #f2f2f2'}}>
    ...
    <div style={{marginTop:20}}><h3 style={{fontSize:25,fontFamily:'cursive'}}>Application Leaves</h3></div> 
<div className='cantainer'  style={{border:'inline',width:1000,marginLeft:3,backgroundColor:'#f2f2f2'}}>   
           ...
           <option>
            {item}
                ...
                <td>
                   ...
                   <button name='Btn_admindashoard_edituser'
...
<div style={{marginTop:20}}><h3 style={{fontSize:25,fontFamily:'cursive'}}>Reporting </h3></div> 
<div className='cantainer'  style={{border:'inline',width:1000,marginLeft:3,backgroundColor:'#f2f2f2'}}>   
<table className="table table-sm">
  <thead >
    <tr style={{backgroundColor:'#8275a5',color:'white'}}>
  ...
  {reportings.map((r) => (
  <tr key={r.reportid}> {/* Adding a unique key for each iteration */}
    <td>{r.empno}</td>
    <td>{r.firstName}</td>
    <td>{r.timeIn}</td>
    <td>{r.timeOut}</td>
    ...
    <td style={{ color: 'f2f2f2', backgroundColor: '' }} className='dropdown'>
      <select
        style={{ border: 'none', backgroundColor: '#ff9999', fontSize: '18' }}
        value={r.status} 
        onChange={(event) => {
